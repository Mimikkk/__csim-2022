...
def inverse_sinogram(sinogram: ndarray[(int, int), number], grayscale: ndarray[(int, int), number],
  ...
  for (i, (line, rotation)) in enumerate(zip(sinogram, linspace(0, tau, scans))):
    ...
    context.text((6, 6), f"RMSE: {rmse(rescale_array(grayscale), rescale_array(reconstruction)):.2f}", fill=200)
    animation[i, :, :] = image_to_array(image)

  return (rescale_array(reconstruction), animation, rmse(rescale_array(grayscale), rescale_array(reconstruction)))
